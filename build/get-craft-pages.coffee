###
Query for all Craft pages given an array of _typenames
###
path = require 'path'
memoize = require 'lodash/memoize'
flatten = require 'lodash/flatten'
getCraftPages = require path.join process.cwd(), '/queries/compiled/craft-pages'
{ getEntries } = require '../services/craft'
{ isGenerating } = require '../config/utils'

# Make the list of routes, memoizing them so they aren't regenerated by sitemap
module.exports = memoize (pageTypes, { generateOnlyRoutes }) ->
	return [] unless isGenerating and pageTypes.length

	# Log start of process
	logLabel = if generateOnlyRoutes then 'routes' else 'data'
	console.log "ℹ Fetching SSG #{logLabel}"

	# Loop through type combinations and add to
	routes = []
	for typename in pageTypes
		results = if generateOnlyRoutes
		then await getRoutesForTypename typename
		else await getEntriesForTypename typename

		# Make the list of routes
		routes = [
			...routes,
			...flatten(results).map (entry) ->

				# Craft uses the `__home__` slug for the homepage
				route: if entry.uri == '__home__' then '/' else "/#{entry.uri}"

				# Look for robots data within a Supertable instance called `seo` or
				# in a robots field directly on the entry
				robots: entry.seo?[0]?.robots || entry.robots || []

				# Wrap in array, so same as asyncData's query
				payload: [ entry ] unless generateOnlyRoutes
		]

	# Return final routes
	console.log "✔ Fetched SSG #{logLabel}"
	return routes

# Get just routes and robots for a pageTypeName
getRoutesForTypename = (typename) ->
	getEntries
		variables: parseTypename typename
		query: '''
			query($section:[String], $type:[String], $site:[String]) {
				entries(section:$section, type:$type, site:$site) {
					uri
					robots
				}
			}
		'''

# Get all page data for all entries given the typename
getEntriesForTypename = (typename) ->
	getEntries
		variables: parseTypename typename
		query: getCraftPages


# Make the query parts for a typename
parseTypename = (typename) ->
	[ section, type ] = typename.split '_'
	{ section, type }
